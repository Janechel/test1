...
struct servo_sync_parameter
{
    ...
    uint8_t id[10];                     //同步写操作舵机的id
    uint16_t position[10];              //舵机位置
    uint16_t time[10];                  //舵机运行时间
    uint16_t velocity[10];              //舵机运动速度
    uint16_t acc_velocity[10];          //舵机运动加速度
    uint16_t dec_velocity[10];          //舵机运动减速度
    uint16_t acc_velocity_grade[10];    //舵机控时下的加速度等级
};
